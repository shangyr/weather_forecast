...
class Worker():
    ...
    def __init__(self):
        ...
        self.take_task_pool = redis.ConnectionPool(host=self.config['redis_address'], port=self.config['redis_port'],
                                                   db=7, max_connections=10)
        # db 8  处理数据队列
        self.deal_data_pool = redis.ConnectionPool(host=self.config['redis_address'], port=self.config['redis_port'],
                                                   db=8, max_connections=10)
        # db 9 保存数据队列 保存分布式数据
        self.save_data_pool = redis.ConnectionPool(host=self.config['redis_address'], port=self.config['redis_port'],
                                                   db=9, max_connections=10)
        # db 10  最终分布式数据保存位置
        self.final_save_data_pool = redis.ConnectionPool(host=self.config['redis_address'],
                                                         port=self.config['redis_port'], db=10, max_connections=10)
        #
        self.timer = redis.Redis(host=self.config['redis_address'], port=self.config['redis_port'], db=11,
                                 max_connections=20)
        # user-agent  太长不要展开
        self.User_agent = [
    ...
    def requests_weather(self, city_list):
        ...
        for city in city_list:
            ...
            proxies = {
                "http": "http://%(user)s:%(pwd)s@%(proxy)s/" % {"user": self.proxy_username, "pwd": self.proxy_password,
                                                                "proxy": self.tunnel},
                "https": "http://%(user)s:%(pwd)s@%(proxy)s/" % {"user": self.proxy_username,
                                                                 "pwd": self.proxy_password, "proxy": self.tunnel}
            }
            try:
