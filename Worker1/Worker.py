...
class Worker():
    """
    爬虫实际执行者，一个worker部署在一台机器上，将使用多线程采集数据
    """
    def __init__(self):
        self.config,self.machineId= self.load_config()
        self.proxy_username,self.proxy_password,self.tunnel = self.load_proxy()
        # print(self.config['thread_count'])
        # print(type(self.config['thread_count']))
        #创建redis连接池
        #take_task_pool获取任务连接池，通过db来区分，实际应用中应多台机器部署不同redis集群 max_connections应根据机器数量和性能调整
        self.take_task_pool = redis.ConnectionPool(host=self.config['redis_address'], port=self.config['redis_port'], db=7, max_connections=10)
        #db 8  处理数据队列
        self.deal_data_pool = redis.ConnectionPool(host=self.config['redis_address'], port=self.config['redis_port'], db=8, max_connections=10)
        #db 9 保存数据队列 保存分布式数据
        self.save_data_pool = redis.ConnectionPool(host=self.config['redis_address'], port=self.config['redis_port'], db=9, max_connections=10)
        #db 10  最终分布式数据保存位置
        self.final_save_data_pool = redis.ConnectionPool(host=self.config['redis_address'], port=self.config['redis_port'], db=10, max_connections=10)
        #
        self.timer = redis.Redis(host=self.config['redis_address'], port=self.config['redis_port'], db=11, max_connections=20)
        #user-agent  太长不要展开
        #设置user-agent
        self.User_agent = [
    ...
    def load_config(self):
        with open("config.json","r",encoding="utf8") as f:
            config = json.load(f)
        with open("machineId.json","r",encoding="utf8") as f:
            machineId = json.load(f)['machineId']
        return config,machineId

    #加载代理信息
    def load_proxy(self):
        with open("proxy.json","r",encoding="utf8") as f:
            config = json.load(f)
        return config['username'],config['password'],config['tunnel']

    #转换数据
    def convert_redisdata(self,redis_data):
        return json.loads(redis_data.decode("utf8"))

    #执行请求天气数据
    def requests_weather(self,city_list):
        #计算消耗时间，由于 请求，解析，保存这些任务是并行的，
        # 所以只需统计出耗时最久的便是当前机器解析获取到城市任务的时间，又由于不同机器间也是并行执行的，所以只需统计出不同机器的最长时间，便是执行这些任务的最长时间
        # start = time.time()
        for city in city_list:
            r = redis.Redis(connection_pool=self.deal_data_pool)
            headers = {
                ...
                "Sec-Fetch-Site": "none",
                "Sec-Fetch-User": "?1",
                "Upgrade-Insecure-Requests": "1",  # User_agent[random.randint(0,999)]
            ...
            proxies = {
                "http": "http://%(user)s:%(pwd)s@%(proxy)s/" % {"user": self.proxy_username, "pwd": self.proxy_password, "proxy": self.tunnel},
                "https": "http://%(user)s:%(pwd)s@%(proxy)s/" % {"user": self.proxy_username, "pwd": self.proxy_password, "proxy": self.tunnel}
            }
            try:
                ...
                if response.text:
                    print("请求数据任务")
                    # 丢到parse_task队列中，机器id + parse_task
                    dat = response.text + "qwertyuioplkjhgfdsazxcvbnm"+city
                    try:
                        # print(dat)
                        r.lpush(self.machineId + "parse_task", json.dumps(dat))
                        # r.lpush(self.machineId + "parse_task", json.dumps(response.text))
                        ################################请求数据任务核心逻辑###########################################
        ...
        # cast_time = end - start
        # # 将天气从redis中取出
        # time_rede = self.timer.get(self.machineId + "reqTime")
        # if not time_rede:
        #     # 第一次请求，为空，
        #     self.timer.set(self.machineId + "reqTime", str(cast_time))
        # else:
        #     time_red_f = float(time_rede)
        #     self.timer.set(self.machineId + "reqTime", str(cast_time + time_red_f))
        # print(f"请求天气任务花费{cast_time}s")
            ################################请求数据任务核心逻辑###########################################
    ...
    def parse_data(self):
        r = redis.Redis(connection_pool=self.deal_data_pool)
        save_red = redis.Redis(connection_pool=self.save_data_pool)
        while True:
            time.sleep(1)
            try:
                redis_data = r.rpop(self.machineId + "parse_task")
                if redis_data:
                    try:
                        start = time.time()
                        ################################解析数据任务核心逻辑###########################################
                        print("解析数据任务")
                        resp = self.convert_redisdata(redis_data)
                        #解析数据
                        query = resp.split("qwertyuioplkjhgfdsazxcvbnm")[1]
                        weather_data = json.loads(re.findall("window.tplData = (.*?);</script>", resp)[0])
                        if weather_data:
                            print(weather_data)
                            weather_data['query'] = query
                            save_red.lpush(self.machineId +"save_task", json.dumps(weather_data))
                            print("解析数据成功")
                    except Exception as e:
                        print(e)
                    finally:
                        cast_time = time.time() - start
                        time_redee = self.timer.get(self.machineId + "parseTime")
                        if not time_redee:
                            # 第一次请求，为空，
                            self.timer.set(self.machineId + "parseTime", str(cast_time))
                        else:
                            time_red_f = float(time_redee)
                            self.timer.set(self.machineId + "parseTime", str(cast_time + time_red_f))
                        print(f"该解析任务花费{cast_time}s")
                        ################################解析数据任务核心逻辑###########################################
            except Exception as e:
                print(e)
                print("解析数据任务异常")
    ...
    def save_data(self):
        save_red = redis.Redis(connection_pool=self.save_data_pool)
        final_save_red = redis.Redis(connection_pool=self.final_save_data_pool)
        while True:
            time.sleep(1)
            try:
                redis_data = save_red.rpop(self.machineId + "save_task")
                if redis_data:
                    try:
                        ################################保存队列任务核心逻辑###########################################
                        start = time.time()
                        print("保存任务")
                        weather_data = self.convert_redisdata(redis_data)
                        print(weather_data)
                        city = weather_data['query']
                        current_date = str(datetime.date.today())
                        key = city + current_date
                        try:
                            #存入 5数据库
                            final_save_red.set(key, json.dumps(weather_data))
                            print(f"{city}天气保存数据成功")
                        ################################保存队列任务核心逻辑###########################################
                        except Exception:
                            print("最终保存数据失败")
                        finally:
                            cast_time = time.time() - start
                            time_rede = self.timer.get(self.machineId + "saveTime")
                            if not time_rede:
                                # 第一次请求，为空，
                                self.timer.set(self.machineId + "saveTime", str(cast_time))
                            else:
                                time_red_f = float(time_rede)
                                self.timer.set(self.machineId + "saveTime", str(cast_time + time_red_f))
                            print(f"保存任务花费{cast_time}s")
                    except Exception:
                        print("保存队列任务异常")
            except Exception as e:
                # print(e)
                pass
    def average_group(self,lst,num):
        """
        将列表均匀分组
        :param lst: 待分配的列表
    ...
    def run(self):
        print("t_parse_data，t_save_data进程启动成功")
        r = redis.Redis(connection_pool=self.take_task_pool)
        # 循环，获取任务  #从redis队列中取出发布的任务
        while True:
            time.sleep(1)
            try:
                redis_data = r.rpop(self.machineId)
                if redis_data:
                    print("获取到城市任务")
                    task_list = self.convert_redisdata(redis_data)
                    #安照线程数量平均分任务
                    print(task_list)
                    print(self.config['thread_count'])
                    print(type(self.config['thread_count']))
                    city_task_list = self.average_group(task_list,self.config['thread_count'])
                    print(city_task_list)
                    threads = []
                    start = time.time()
                    for i in range(self.config['thread_count']):
                        t = threading.Thread(target=self.requests_weather, args=(city_task_list[i], ))
                        t.start()
                        threads.append(t)
                    for t in threads:
                        t.join()
...
if __name__ == '__main__':
    ...
    #创建5个parse_data进程
    for i in range(5):
        t = threading.Thread(target=wk.parse_data)
        t.daemon = False
        t.start()
    #save_data
    for i in range(5):
        t = threading.Thread(target=wk.save_data)
        t.daemon = False
        t.start()
    # p_parse_data = threading.Thread(target=wk.parse_data)
    # p_save_data = threading.Thread(target=wk.save_data)
    # p_parse_data.daemon = False
    # p_save_data.daemon = False
    # p_parse_data.start()
    # p_save_data.start()
    if test:
        wk.test()
